...
export class SearchableProjectListService {
  ...
  allProjects$ = combineLatest([
    ...
    switchMap(([searchText, loadingEnabled, favoriteIds]:[string,boolean,string[]]) => {
      ...
      const nameFilter = [] as ApiV3ListFilter[];
      if (searchText.length > 0) {
        nameFilter.push(['name', '~', [searchText]]);
      }

      return this.http.get<IHALCollection<IProject>>(this.apiV3Service.projects.path + listParamsString(this.params(nameFilter)))
    ...
    switchMap((projects) => {
      return this.pipeConcatProjects(projects, this.extractAncestors(projects));
    })
  );
   ...
   * @param {string[]} extraIds - A list of project IDs that identifies projects that shall be appended
   */
  private pipeConcatProjects(projects:IProject[], extraIds:string[]) {
    const existingIds = this.extractIds(projects);
    extraIds = extraIds.filter((id) => !existingIds.has(id));

    const extraFetches = [] as Observable<IHALCollection<IProject>>[];
    for(let sliceStart = 0; sliceStart < extraIds.length; sliceStart += this.maximumPageSize) {
      const extraFilter = [['id', '=', extraIds.slice(sliceStart, sliceStart + this.maximumPageSize)]] as ApiV3ListFilter[];
      extraFetches.push(
        this.http.get<IHALCollection<IProject>>(this.apiV3Service.projects.path + listParamsString(this.params(extraFilter, this.maximumPageSize)))
      );
    }
